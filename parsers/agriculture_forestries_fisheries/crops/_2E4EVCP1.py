...
def build_production_rdf(headers, lines, places_to_uri_table, places_uri_to_nodes_table, crops_to_uri_table, crops_uri_to_nodes_table, rdf_graph):
    ...
    for line in lines:
        ...
        for i in range(2, len(headers)):
            year = headers[i]
            production_area = trim(line[i])
            if len(production_area) > 0:
                production_area = float(production_area)
                ...
                rdf_graph.add((production_node, production_namespace.productionArea, Literal(production_area)))
